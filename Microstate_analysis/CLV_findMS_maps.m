...
for counter = 1 : numel(SelectedSets)
    ...
    assignin('base', 'MapsToUse', MapsToUse)
    msinfo.Maps_per_segment = MapsPerSegment;
    msinfo.GFP = gfp;
    msinfo.Is_GFP_peak = IsGFPPeak;
    msinfo.Maps_to_use = MapsToUse;
    ...
    if ClustPar.UseAAHC == false
        figure;
        rows = 2;
        cols = ceil(numel(ClustPar.MinClasses:ClustPar.MaxClasses)/rows);
     
        counter = 0;
        for nClusters = ClustPar.MinClasses:ClustPar.MaxClasses
            counter = counter +1;
            [b_model,b_ind,b_loading,exp_var] = eeg_kMeans(MapsToUse',nClusters,ClustPar.Restarts,[],flags,DataIn(currSetIndx).chanlocs);
            
            assignin('base', 'b_model', b_model)
                        
            BModel{counter,1} = b_model;
            BInd{counter,1}   = b_ind;
            BLoading{counter,1} = b_loading;

            silhMean(counter) = CLV_plotSilhouette(MapsToUse', b_ind, rows, cols, counter, nClusters); % This means doesn't work for the modified Kmeans.
            ...
            msinfo.MSMaps(nClusters).B_ind = BInd{counter,1};
            msinfo.MSMaps(nClusters).B_loading = BLoading{counter,1};
            ...
            msinfo.MSMaps(nClusters).SilhouetteMean = silhMean(counter);
...
assignin('base', 'b_model', b_model)
assignin('base','b_ind',b_ind)
assignin('base', 'b_loading', b_loading)
assignin('base','BModel',BModel)
assignin('base', 'BInd', BInd)
assignin('base','BLoading', BLoading)
