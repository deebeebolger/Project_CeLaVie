...
if ~isempty(PubSet)
    templatePath = '/Users/bolger/Documents/MATLAB/Projects/CeLaVie_EEG/Data/clv_templates/';
    ...
    elseif sum(ismember(templateNames_all, [PubSet,'.set'])) == 0
        error(['The following published template sets could not be found in the Templates folder.\n']);
        PublishedSetIndices = [];
    end
...
if compWithin
    ...
    for i=1:numel(SelectedEEG)
        ...
        if isempty(SelectedEEG(i).msinfo.MSMaps(Classes).Maps)     % Implies in our case that there is only 7-cluster solution.

            currclass = find(~cell2mat(cellfun(@(x) isempty(x), {SelectedEEG(i).msinfo.MSMaps.Maps}, 'UniformOutput',false)));
            find_commclass = ismember(SelectedEEG(i).msinfo.MSMaps(currclass).Labels, SelectedEEG(i-1).msinfo.MSMaps(Classes).Labels);
            MapCollection = [MapCollection; SelectedEEG(i).msinfo.MSMaps(currclass).Maps(find_commclass,:)];
        else
        ...
        for j=1:Classes
            ...
            nameSplit = split(SelectedEEG(i).setname, '_');
           
            if numel(nameSplit)>=5                          % Implies generally that this is the Grandmean microset 
                SelectedEEG_name = 'GrandMean';
                SelectedEEG(i).setname = 'GrandMean';
            else
                SelectedEEG_name = SelectedEEG(i).setname;            
            end
            
            if isempty(find_commclass)
                CLabelCollection = [CLabelCollection, sprintf("%s (%s)", SelectedEEG(i).msinfo.MSMaps(Classes).Labels{j}, SelectedEEG_name)];
            else
                classint = find(find_commclass);
                CLabelCollection = [CLabelCollection, sprintf("%s (%s)", SelectedEEG(i).msinfo.MSMaps(currclass).Labels{classint(j)}, SelectedEEG_name)];
            end
        end
    end
end
...
%% If necessary, correct the SelectedEEG(2) structure so that the number of maps is consistent across the
%  the selectedEEG sets.

if ~isempty(classint)
    SelectedEEG(2).msinfo.MSMaps(currclass).Maps = SelectedEEG(2).msinfo.MSMaps(currclass).Maps(classint,:);
    SelectedEEG(2).msinfo.MSMaps(currclass).ColorMap = SelectedEEG(2).msinfo.MSMaps(currclass).ColorMap(classint,:);
    SelectedEEG(2).msinfo.MSMaps(currclass).Labels   = SelectedEEG(2).msinfo.MSMaps(currclass).Labels(classint);
    SelectedEEG(2).msinfo.ClustPar.MinClasses = numel(classint);
    SelectedEEG(2).msinfo.ClustPar.MaxClasses = numel(classint);
    SelectedEEG(2).msinfo.MSMaps(Classes) = SelectedEEG(2).msinfo.MSMaps(currclass);
end
