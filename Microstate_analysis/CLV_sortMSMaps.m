...
elseif isstruct(EEGIn{1,1})
    ...
    % Test the individual-level or group/condition-level datasets to be sorted
...
AvailableIndSets = AvailableSets_Ind(~HasChildren1);  % Find the individual datasets - those that do not have children. If empty it means that the dataset to be sorted is a group- or condition-level mean dataset.
AvailableMeanSets = AvailableSets_Temp(HasChildren2);  % Find the grandmean datasets - those with children.
...
if strcmp(TemplateSet, 'GMdataset')
    ...
    elseif sum(cell2mat(arrayfun(@(x) isfield(EEGIn{1,1}(x).msinfo, 'children'), 1:numel(EEGIn{1,1}), 'UniformOutput',false))) == numel(EEGIn{1,1})

         arrayfun(@(x) fprintf('The current dataset to be sorted : %s is a group- or condition-level dataset.\n', EEGIn{1,1}(x).setname), 1:numel(EEGIn{1,1}), 'UniformOutput', false)
         fprintf('Checking if is one of the children of the grand-mean template set...\n');
         ischildren = cell2mat(arrayfun(@(x) sum(ismember(gmchildren{1,1}, EEGIn{1,1}(x).msinfo.children)), 1:numel(EEGIn{1,1}), 'UniformOutput',false));

         if cell2mat(arrayfun(@(y) ischildren == numel(EEGIn{1,1}(y).msinfo.children), 1:numel(EEGIn{1,1}), 'UniformOutput',false))
             fprintf('The group/condition-level mean dataset, %s, has children in common with those of the grand-mean template dataset.\n',EEGIn{1,1}.setname)
         end
...
%     if isfield(ChosenTemplate, 'msinfor')
%         sortmode_indx = cell2mat(cellfun(@(x) ~isempty(x), {ChosenTemplate.msinfor.MSMaps.SortMode},'UniformOutput', false));
%         SortMode = ChosenTemplate.msinfor.MSMaps(sortmode_indx).SortMode;
% 
%         if matches('none', SortMode)
%             errorMessage = sprintf('Selected template solution %i of template set %s is unsorted. Please sort before using as a template solution.', TemplateClassNum, TemplateName);
%             error(errorMessage);
%         end
%     else
%         TemplateName = ChosenTemplate.setname;
%         errorMessage = sprintf('Selected template solution %i of template set %s is unsorted. Please sort before using as a template solution.', TemplateClassNum, TemplateName);
%         error(errorMessage);
%     end
% 
...
if Stepwise
    ...
    for i = 1:length(selectSets)
        ...
        savepath = '/Users/bolger/Documents/MATLAB/Projects/CeLaVie_EEG/Data/Microstate/MS_Posttest_SingleSubject_Level/Posttest_reordered_grandmean_template_6class';
        EEG = pop_saveset(EEGIn{1,1}(sIndex), 'filename', [EEGIn{1,1}(sIndex).setname,'_sortedPublished_metatemplate_v2'], 'filepath', savepath);
