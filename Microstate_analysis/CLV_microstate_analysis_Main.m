function CLV_microstate_analysis_Main()
%% ***************************************************************************************
...
analysisType = 'find_Microstates';    % Or 'find_Microstates'  'sortMaps' 'combine_Microstates'
...
if strcmp(analysisType, 'find_Microstates')
 ...
 %% Carry out the microstate segmentation of the individual datasets.
    ...
    for counter = 1:numel(SelectedSets)
        currfilename = [DataOut(counter).setname, '_MSMaps_v2'];
...
elseif strcmp(analysisType, 'combine_Microstates')
    ...
    AverageAcross = {{'Test'}, {'posttest'}, {'restingstate1_', 'restingstate2'}};    % Averaging group, condition and restingstate criteria.
    ...
    classes = [EEGOUT(1).msinfo.ClustPar.MinClasses:EEGOUT(1).msinfo.ClustPar.MaxClasses];

    if ShowMaps

        fig_h = arrayfun(@(a) pop_ShowIndMSMaps(a, 1:numel(a), 'Classes', classes, 'Visible', 0), EEGOUT, 'UniformOutput', false);
        savepath = {EEGOUT.filepath};
        figname  = {EEGOUT.setname};
